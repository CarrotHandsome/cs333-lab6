...
eoraptor_server: eoraptor_server.o
	gcc $(CFLAGS) -o eoraptor_server eoraptor_server.o -lncurses

eoraptor_server.o: eoraptor_server.c eoraptor_ui.h
	gcc $(CFLAGS) -c -o eoraptor_server.o eoraptor_server.c
...
run-clients:
	...
	tmux new-session -d -s clients -x "$$(tput cols)" -y "$$(tput lines)" \; \
	split-window \; \
	split-window \; \
	split-window \; \
	select-layout tiled \; \
	send-keys -t 0 './eoraptor_client -P $(PORT)' Enter \; \
	send-keys -t 1 './eoraptor_client -P $(PORT)' Enter \; \
	send-keys -t 2 './eoraptor_client -P $(PORT)' Enter \; \
	send-keys -t 3 './eoraptor_client -P $(PORT)' Enter; \
